...
def draw_bboxes(img, bbox, identities=None, offset=(0,0), dists):
    myoffset = -50
    for i,box in enumerate(bbox):
        ...
        print("id: " + identities[i] + " | " + "dist = " + dists[i])
        color = COLORS_10[id%len(COLORS_10)]
        label = '{} {}'.format("Boat", id)
...
def distance_calc(B_size):
    B_temp = pow(B_size, 2) + pow(13, 2)
    dist_temp = math.sqrt(B_temp)
    mvar = dist_temp
    fdistance = 0
    tvar = 0
    if mvar < tvar: 
        diff = (tvar - mvar) * 8
        fdistance = (mvar * 8) - diff 
        tvar = dist_temp
    else:
        mvar = dist_temp
        fdistance = (mvar * 8)
